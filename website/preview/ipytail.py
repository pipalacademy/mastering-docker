...
class IPyTail:
    ...
    def process_file(self, filename, skip_till=None, tail=True):
        nb = self.read_notebook(filename)
        if skip_till:
            nb = self.skip_till(nb, skip_till)
        if tail:
            nb = self.tail(nb, self.max_cells)
        nb = self.trim_notebook_outputs(nb, self.max_lines)
        return nb

    def skip_till(self, nb, title):
        headers = {cell['source'][0].strip().strip("#").strip(): i for i, cell in enumerate(nb['cells'])
        ...
        if title in headers:
            index = headers[title]
            return dict(nb, cells=nb['cells'][index:])
        else:
            return nb
